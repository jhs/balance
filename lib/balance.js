...
Balancer.prototype.getRequestHandler = function (clientRequest, clientResponse) {
  ...
  return function (clientRequest, clientResponse) {  
    ...
    self.emit("route", clientRequest, function do_route (port, hostname, host_header) {
      sys.puts('Routing...');
      route = {port: port, hostname: hostname, host_header: host_header};
      sys.puts('route = ' + sys.inspect(route));

      if (route.finished) {
        sys.puts('No additional routing');
        return;
      }
      ...
      // XXX HERE: figure out how to get that c.request to call back.
      
      sys.puts('connecting to '+route.hostname+':'+route.port+' for ' + clientRequest.url);
      var proxyRequest = c.request(clientRequest.method, clientRequest.url, clientRequest.headers);
      sys.puts('proxyRequest = ' + sys.inspect(proxyRequest));
